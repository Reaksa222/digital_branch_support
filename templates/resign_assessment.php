...
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
    ...
    // $sql = "SELECT display_name, brunch,position, function, role, status ,requester, approver,end_date,comment FROM tbl_create_user WHERE id = ?";
    $sql = "SELECT  display_name, function, role, brunch, position, status, approver, comment FROM summary WHERE id=?";
    ...
    if ($stmt->execute()) {
        ...
        if ($stmt->num_rows > 0) {
            $stmt->bind_result($display_name, $function, $role,$brunch, $position, $status, $approver, $comment);
            $stmt->fetch();
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // $sql = "UPDATE tbl_create_user SET display_name = ?, brunch =? ,position = ?, function =? ,role =?, status = ?, approver =?,  end_date =? ,comment =? WHERE id = ? ";
    $sql = "INSERT INTO tbl_resign_user (display_name,brunch, position, function, role, status, approver, end_date, comment) VALUES ( ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = $conn->prepare($sql);
    ...
    $stmt->bind_param("sssssssss", $display_name, $brunch, $position, $function, $role, $status, $approver, $end_date, $comment);
