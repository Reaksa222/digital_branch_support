...
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
    ...
    $sql = "SELECT  display_name, function, role, brunch, position, status,requester, approver,comment FROM summary WHERE id=?";
    ...
    if ($stmt->execute()) {
        ...
        if ($stmt->num_rows > 0) {
            $stmt->bind_result($display_name, $function, $role,$brunch, $position, $status,$requester, $approver, $comment);
            $stmt->fetch();
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "INSERT INTO tbl_move_user (display_name, brunch, position, function, role, status, requester, approver, start_date, end_date, comment) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
